...
def main(args):
    ...
    while train:
        for i, data in enumerate(trainloader, 1):
            ...
            rec_loss, latent_loss, x_hat = vae(x_in)
            loss_ = rec_loss+ latent_loss
            loss = loss_.mean() - log_det
            running_loss += loss.item()
            rec_losses += rec_loss.mean()
            latent_losses += latent_loss.mean()
            ...
            if total_iter % 1000 == 0:
                ...
                mean_rec_loss = rec_losses / 1000
                mean_latent_loss = latent_losses / 1000
                bit_per_dim = (float(loss) + np.log(256.) * C*H*W) \
                            / (C*H*W * np.log(2.))
                print('iter %s:' % total_iter,
                      'loss = %.3f' % mean_loss,
                      'rec = %.3f' % mean_rec_loss,
                      'latent_loss = %.3f' % mean_latent_loss,
